...
class CosmicFengine():
    ...
    def set_lo_fshift_list(self, lo_fshift_list, skip_if_equal=False):
        ...
        if lo_load_time > time.time():
            ...
            if not np.isclose(fshift_time_to_load, 0.0, atol=1e-1):
                self.logger.warning(f"After sleeping, time to load from the F-Engine {fshift_time_to_load}s is not near zero.")
        else:
        ...
        while self.delay_tracking_switch.is_set():
            ...
            if message and "message" == message["type"]:
                ...
                try:
                    ...
                    if not np.isclose(feng_time_to_load, expected_sleep_duration, atol=1e-3):
                            self.logger.warn(f"Time to load from the F-Engine {feng_time_to_load}s is not within 1ms of the expected time to load {expected_sleep_duration}s.")
                    if not feng_time_to_load > 0:
